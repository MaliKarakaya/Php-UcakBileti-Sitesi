...
session_start();
	...
	$from = $_POST['from'];
	$to = $_POST['to'];
	$flight_Instance = rand(2000,3000);
	echo $flight_Instance;
	...
	|| empty($seats) || empty($depart_date) || empty($arrive_date) || empty($fromCity) 
	...
	else
	{
		...
		if(mysqli_num_rows($query) > 0)
		{
			...
			if (mysqli_query($con, $sql4) && mysqli_query($con, $sql5)) 
				...
				{
					$_SESSION['error_msg'] = "Üzgünüz , Uçuş eklenemedi .";
					header('Location: errorPage.php');
				}
